...
namespace towr
{
    ...
    public:
        ...
        Eigen::Matrix<double, kJointLegNum, 1> Geomotory_and_InverseKinematics(Eigen::Vector3d target_position, Eigen::Vector3d base_position, Eigen::Matrix3d R_base_to_hip, int hip_id, int robot_type);
        void EnforceAngleLimits(double &val, double max, double min) const;
        Eigen::Vector3d ComputeJacobian_and_ForwardKinematics(Eigen::Matrix<double, kJointLegNum, 1> joint_angles, Eigen::Matrix<double, kJointLegNum, kJointLegNum> &Jacobian_Matrix, int hip_id, int robot_type);
        Eigen::Vector3d TransformWorldFrameToHipFrame(Eigen::Vector3d foot_position_in_world_frame, Eigen::Vector3d base_position_in_world_frame, Eigen::Matrix3d Rot_matrix, int hip_id, int robot_type);
