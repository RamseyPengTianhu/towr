...
struct RBDL_DLLAPI Body {
  ...
  void Join (const Math::SpatialTransform &transform, const Body &other_body)
  {
    ...
    RBDL_LOG << "inertia_other = " << std::endl << inertia_other << std::endl;
    ...
    RBDL_LOG << "inertia_other_com = " << std::endl << inertia_other_com << std::endl;

    // 2. Rotate the inertia that it is aligned to the frame of this body
    Math::Matrix3d inertia_other_com_rotated = transform.E.transpose() *
        inertia_other_com * transform.E;
    RBDL_LOG << "inertia_other_com_rotated = " << std::endl << inertia_other_com_rotated
        << std::endl;
    ...
    RBDL_LOG << "inertia_other_com_rotated_this_origin = " << std::endl <<
